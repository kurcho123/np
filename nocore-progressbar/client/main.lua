...
--[[
RegisterCommand("testprogressbar", function(source, args)
    local keyboard = exports["nocore-keyboard"]:KeyboardInput({
        header = "Test Icon",
        rows = { { id = 0, label = "Icon" } }
        })

    if keyboard and keyboard[1] then
        local icon = tostring(keyboard[1].input)
...
function Process(action, start, tick, finish)
    ...
    if Action.icon then
		if Framework.Shared.Items[tostring(Action.icon)] then
			local img = "nui://nocore-inventory/html/" -- default qb-core inventory link
			if not string.find(Framework.Shared.Items[tostring(Action.icon)].image, "http") then -- 👀 Slipped in support for custom html links too
				if not string.find(Framework.Shared.Items[tostring(Action.icon)].image, "images/") then --search for if the icon images have /images in the listing
					img = img.."images/"
				end
				Action.icon = img..Framework.Shared.Items[tostring(Action.icon)].image
			end
		end
	end
    ...
    if not IsEntityDead(PlayerPedId()) or Action.useWhileDead then
        if not isDoingAction then
            isInventoryDisabled = Action.controlDisables.disableInventory or false
            isDoingAction = true
            wasCancelled = false
            isAnim = false
            isProp = false
            ...
            Framework.Functions.Notify("Вече правиш нещо!", "error")
        end
    else
        Framework.Functions.Notify("Не може да се извърши това действие!", "error")
    end
end
