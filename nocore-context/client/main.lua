...
local function openMenu(data)
	...
	for _,v in pairs(data) do
		if v["icon"] then
			local img = "nocore-inventory/html/"
			if Framework.Shared.Items[tostring(v["icon"])] then
				if not string.find(Framework.Shared.Items[tostring(v["icon"])].image, "images/") then
					img = img.."images/"
				end
				v["icon"] = img..Framework.Shared.Items[tostring(v["icon"])].image
			end
		end
	end
        ...
        if data then
            if data.params.event then
                ...
                elseif data.params.isQBCommand then
                    TriggerServerEvent('Framework:CallCommand', data.params.event, data.params.args)
