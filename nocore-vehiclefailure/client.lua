	...
	while true do
		...
		if isPedDrivingAVehicle() then
			...
			if pedInSameVehicleLast == true then
				...
				--defaultHandling = GetVehicleHandlingFloat(vehicle, 'CHandlingData', 'fSteeringLock')
				...
				--SetVehicleHandlingFloat(vehicle, 'CHandlingData', 'fSteeringLock', defaultHandling * 0.8)
...
RegisterNetEvent('nocore-vehiclefailure:client:fix:veh')
AddEventHandler('nocore-vehiclefailure:client:fix:veh', function()
	if IsPedSittingInAnyVehicle(PlayerPedId()) then
		...
		SetVehicleFuelLevel(Vehicle, Amount + 0.0)
 		TriggerServerEvent('nocore-fuel:server:update:fuel', vehiclePlate, Vehicle, math.floor(Amount))
	else
		local Vehicle = Framework.Functions.GetClosestVehicle()
		SetVehicleFixed(Vehicle)
	end
	-- Framework.Functions.Notify("Автомобилът е ремонтиран", 'success')
end)