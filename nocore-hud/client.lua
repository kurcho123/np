...
local function hasHarness()
    ...
    local hasHarness = exports['nocore-assets']:HasHarness()
    if hasHarness then
        _harness = true
    else
        _harness = false
...
local function loadSettings()
    Framework.Functions.Notify(Lang:t("notify.hud_settings_loaded"), "success")
    Wait(1000)
...
local function HandleSetupResource()
    Framework.Functions.TriggerCallback('hud:server:getRank', function(isAdminOrGreater)
...
RegisterNetEvent("Framework:Client:OnPlayerLoaded", function()
    ...
    hunger, thirst, drunk, stress = PlayerData.metadata["hunger"], PlayerData.metadata["thirst"], PlayerData.metadata["drunk"], PlayerData.metadata["stress"]
    TriggerEvent('nocore-hud:client:update:drink:level', drunk)
end)
...
RegisterCommand('hudoptions', function ()
    Wait(50)
    if showMenu then return end
    TriggerEvent("hud:client:playOpenMenuSounds")
    SetNuiFocus(true, true)
...
local function restartHud()
    ...
    Framework.Functions.Notify(Lang:t("notify.hud_restart"), "error")
    Wait(1500)
    ...
    Wait(500)
    Framework.Functions.Notify(Lang:t("notify.hud_start"), "success")
    SendNUIMessage({
        action = 'menu',
        topic = 'restart',
    })
...
RegisterNUICallback('resetSettings', function(_, cb)
    ...
    if Menu.isResetSoundsChecked then
        TriggerServerEvent("nocore-sound:server:play:source", "airwrench", 0.1)
    end
    Wait(2500)
    SendNUIMessage({
        action = 'menu',
...
--[[ RegisterNUICallback('createUISettings', function(data, cb)
    cb({})
    Wait(50)
    SetResourceKvpInt('haveHudSettings', 1)
end) ]]
...
RegisterNetEvent("hud:client:LoadMap", function()
    ...
    if Menu.isToggleMapShapeChecked == "square" then
        ...
        if Menu.isMapNotifChecked then
            Framework.Functions.Notify(Lang:t("notify.load_square_map"))
        end
        ...
        Wait(1200)
        if Menu.isMapNotifChecked then
            Framework.Functions.Notify(Lang:t("notify.loaded_square_map"))
        end
    elseif Menu.isToggleMapShapeChecked == "circle" then
        ...
        if Menu.isMapNotifChecked then
            Framework.Functions.Notify(Lang:t("notify.load_circle_map"))
        end
        ...
        Wait(1200)
        if Menu.isMapNotifChecked then
            Framework.Functions.Notify(Lang:t("notify.loaded_circle_map"))
        end
    end
end)
...
RegisterNUICallback('cinematicMode', function(data, cb)
    ...
    if data.checked then
        ...
        if Menu.isCinematicNotifChecked then
            Framework.Functions.Notify(Lang:t("notify.cinematic_on"))
        end
    else
        CinematicShow(false)
        if Menu.isCinematicNotifChecked then
            Framework.Functions.Notify(Lang:t("notify.cinematic_off"), 'error')
        end
...
RegisterNetEvent('hud:client:UpdateNeeds', function(newHunger, newThirst, newDrunk, newStress) -- Triggered in no-core
    hunger = newHunger or hunger
    thirst = newThirst or thirst
    stress = newStress or stress
    drunk = newDrunk or drunk
    TriggerEvent('nocore-hud:client:update:drink:level', drunk)
end)
...
RegisterNetEvent("no-admin:client:ToggleDevmode", function()
    dev = not dev
end)

exports('devmode', function() return dev end)
...
RegisterNetEvent('hud:client:BuffEffect', function(data)
    ...
    elseif data.display ~= nil then
        ...
        print("nocore-hud error: data invalid from client event call: hud:client:BuffEffect")
    end
end)
...
RegisterNetEvent('hud:client:EnhancementEffect', function(data)
    ...
    elseif data.display ~= nil then
        ...
        print("nocore-hud error: data invalid from client event call: hud:client:EnhancementEffect")
    end
end)
...
RegisterCommand('+engine', function()
    ...
    if GetIsVehicleEngineRunning(vehicle) then
        Framework.Functions.Notify(Lang:t("notify.engine_off"))
    else
        Framework.Functions.Notify(Lang:t("notify.engine_on"))
    end
...
local function getFuelLevel(vehicle)
    ...
    if (updateTick - lastFuelUpdate) > 2000 then
        ...
        lastFuelCheck = math.floor(exports[Config.FuelScript]:GetFuelLevel(GetVehicleNumberPlateText(plyVehicle)))
    end
...
AddEventHandler('baseevents:leftVehicle', function(veh, seat, class, netId)
    plyVehicle = nil
    if Menu.isOutMapChecked then
        SetRadarZoom(1000)
        DisplayRadar( not Menu.isMapEnabledChecked)
    end
end)
...
AddEventHandler('pma-voice:setTalkingMode', function(TalkingMode)
    if TalkingMode == 1 then
        voiceMode = 2
    elseif TalkingMode == 2 then
        voiceMode = 4
    else
        voiceMode = 6
    end
...
CreateThread(function()
    ...
    while true do        
        if LocalPlayer.state.isLoggedIn then
            ...
            playerDead = exports['nocore-hospital']:GetDeathStatus() or false
    ...
    while true do
        if LocalPlayer.state.isLoggedIn then
            ...
            if IsPedInAnyVehicle(ped, false) and not IsThisModelABicycle(GetEntityModel(GetVehiclePedIsIn(ped, false))) then
                if exports[Config.FuelScript]:GetFuelLevel(GetVehicleNumberPlateText(GetVehiclePedIsIn(ped, false))) <= 20 then -- At 20% Fuel Left
                    if Menu.isLowFuelChecked then
                        TriggerServerEvent("InteractSound_SV:PlayOnSource", "pager", 0.10)
                        Framework.Functions.Notify(Lang:t("notify.low_fuel"), "error")
...
RegisterNetEvent('nocore-hud:client:money:change', function(type, amount, isMinus)
...
-- end)
-- RegisterCommand("stress2", function()
--     TriggerServerEvent('hud:server:GainStress', 25)
-- end)
            ...
            if stress >= 100 then
                ...
                if IsInVehicle then
                    ...
                    for i = 1, FallRepeat, 1 do
                        Wait(750)
                        TaskVehicleTempAction(ped, veh, math.random(10,11), math.random(200,500))
                        Wait(1000)
                        TaskVehicleTempAction(ped, veh, math.random(10,11), math.random(200,500))
                        TriggerScreenblurFadeIn(1000.0)
                        Wait(BlurIntensity)
...
CreateThread(function()
	RegisterKeyMapping("seatbelt", 'Car Seatbelt', 'keyboard', "G")
	return
end)
	...
	if vehicle ~= 0 then
        if seatbeltOn then
            SetTimeout(500,function()
                seatbeltOn = false
                TriggerEvent("nocore-sound:client:play", "car-unbuckle", 0.25)
                Framework.Functions.Notify('Seat Belt Disabled', 'info')
                -- SetFlyThroughWindscreenParams(55.6464, 55.9352, 0.0, 0.0)
                -- SetPedConfigFlag(PlayerPedId(), 32, true)
            end)
        else
            SetTimeout(500,function()
                seatbeltOn = true
                TriggerEvent("nocore-sound:client:play", "car-buckle", 0.25)
                Framework.Functions.Notify('Seat Belt Enabled', 'info')
                -- SetFlyThroughWindscreenParams(155.6464, 2.2352, 0.0, 0.0)
                while seatbeltOn do
...
RegisterNetEvent("nocore-hud:client:update:drink:level")
 AddEventHandler("nocore-hud:client:update:drink:level", function(NewDrinkLevel)
     local drinklevel = NewDrinkLevel
     local fade = false
     local level = 0
     if drinklevel >= 2 and drinklevel <= 25 then
         level = 1
     elseif drinklevel >= 26 and drinklevel <= 50 then
         level = 2
     elseif drinklevel >= 51 and drinklevel <= 75 then
         level = 3
     elseif drinklevel >= 76 and drinklevel <= 98 then
         level = 4
     elseif drinklevel >= 99 and drinklevel <= 100 then
         level = 5
     end
     if --[[ LastDrinkLevel == 0 or ]] LastDrinkLevel ~= level or (drinklevel == 0 and LastDrinkLevel == 1) then
         fade = true
         Drunk(level, fade)
     end
     LastDrinkLevel = level
 end)
 
 function Drunk(level, fade)
     ...
     Citizen.CreateThread(function()
         ...
         elseif level == 2 then
             Wait(1000)
             ClearTimecycleModifier()
             SetTimecycleModifier("shades_yellow")
             TriggerEvent('animations:client:WalkCommandStart', {"drunk"})
             ShakeGameplayCam("DRUNK_SHAKE", 1.0)
         elseif level == 3 then
             Wait(1000)
             ClearTimecycleModifier()
             SetTimecycleModifier("DRUG_2_drive")
             TriggerEvent('animations:client:WalkCommandStart', {"drunk2"})
         ...
         elseif level == 5 then
             ...
             TaskPlayAnim(playerPed, 'random@drunk_driver_1', 'drunk_fall_over', 1.0, -1.0, 5000, 0, 1, false, false, false)
 
             Wait(5000)
             Framework.Functions.Notify("Пиенето ви дойде в повече и припаднахте!")
             DoScreenFadeOut(100)
             SetPedToRagdoll(playerPed, 40000, 40000, 0, 0, 0, 0)
             ...
             DoScreenFadeIn(1000)
             TriggerServerEvent("Framework:Server:SetMetaData", "drunk", 90)
         end
