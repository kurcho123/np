...
RegisterCommand('acceptemote', function()
    if not isRequestAnim then return end
    target, distance = GetClosestPlayer()
    if(distance ~= -1 and distance < 3) then
...
RegisterKeyMapping('acceptemote', 'Accept emote request', 'keyboard', 'Y')
TriggerEvent('chat:removeSuggestion', '/acceptemote')

RegisterCommand('refuseemote', function()
    if not isRequestAnim then return end
    SimpleNotify(Config.Languages[lang]['refuseemote'])
    isRequestAnim = false
end, false)
RegisterKeyMapping('refuseemote', 'Refuse emote request', 'keyboard', 'L')
TriggerEvent('chat:removeSuggestion', '/refuseemote')
