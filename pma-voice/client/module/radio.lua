...
function isDead()
	if GetResourceState("nocore-hospital") ~= "missing" then
		if exports['nocore-hospital']:GetDeathStatus() then
			return true
		else
			return false
		end
...
function isHandCuffed()
	if GetResourceState("nocore-police") ~= "missing" then
		if Framework.Functions.GetPlayerData().metadata['ishandcuffed'] then
			return true
		else
			return false
		end
...
RegisterCommand('-radiotalk', function()
	if isHandCuffed() and not radioPressed then return end
...
function LoadRadio()
	...
	SetFilterParameters('radio', Cfg.Submixes['radio'])
end