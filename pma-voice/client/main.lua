...
local volumes = {
	...
	['megaphone'] = tonumber(GetConvar('voice_defaultVolume', '1.0'))
}
plyState = LocalPlayer.state
local micClicks = true
playerServerId = GetPlayerServerId(PlayerId())
radioEnabled, radioPressed, mode, radioChannel, callChannel, megaphone = false, false, 1, 0, 0, false 
...
RegisterNetEvent('pma-voice:logplayer')
AddEventHandler('pma-voice:logplayer', function()
	if GetConvarInt('voice_enableProximity', 1) ~= 1 then return end
	if playerMuted then return end
...
local submixFunctions = {
	['radio'] = function(plySource)
		SetPlayerFilter(plySource, 'radio')
	end,
	['phone'] = function(plySource)
		SetPlayerFilter(plySource, 'phone')
	end,
	['megaphone'] = function(plySource)
		SetPlayerFilter(plySource, 'megaphone')
	end
}
...
function toggleVoice(plySource, enabled, moduleType)
	logger.log("[LOGGER] %s, %s, %s",plySource,enabled,moduleType)
	if enabled then
		logger.log('[main] Updating %s to talking: %s with submix %s, with megaphone variable %s', plySource, enabled, moduleType, megaphone)
		MumbleSetVolumeOverrideByServerId(plySource, enabled and volumes[moduleType])
		--if GetConvarInt('voice_enableSubmix', 1) == 1 or GetConvarInt('voice_enableRadioSubmix', 1) == 1 then
			if moduleType then
...
RegisterCommand('+cycleproximity', function()
	if GetConvarInt('voice_enableProximity', 1) ~= 1 then return end
	if playerMuted then return end
	if megaphone then StopAnimTask(PlayerPedId(), "random@arrests", "generic_radio_enter", -4.0) end
	...
	while true do
		...
		updateZone()
			if lastRadioStatus ~= radioPressed or lastTalkingStatus ~= (NetworkIsPlayerTalking(PlayerId()) == 1) then
				...
				TriggerEvent('pma-voice:setRadioMode', lastTalkingStatus, lastRadioStatus)
			end
		Wait(50)
	end
end)
...
RegisterCommand('vsync', function()
	...
	updateZone(true)
	--print(mode)
	--print(voiceModeData[1])
	--print(voiceModeData[2])
end)
