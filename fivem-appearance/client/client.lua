...
local function RemoveTargets()
    if Config.EnablePedsForShops then
        ...
        for k, v in pairs(Config.Stores) do
            exports['nocore-eye']:RemoveZone(v.shopType .. k)
        end
    end
    ...
    if Config.EnablePedsForClothingRooms then
        ...
        for k, v in pairs(Config.ClothingRooms) do
            exports['nocore-eye']:RemoveZone('clothing_' .. v.requiredJob .. k)
        end
    end
    ...
    if Config.EnablePedsForPlayerOutfitRooms then
        ...
        for k in pairs(Config.PlayerOutfitRooms) do
            exports['nocore-eye']:RemoveZone('playeroutfitroom_' .. k)
        end
    end
end
...
AddEventHandler('onResourceStop', function(resource)
    if resource == GetCurrentResourceName() and GetResourceState("nocore-eye") == "started" then
...
RegisterNetEvent('Framework:Client:SetDuty', function(duty)
    PlayerJob.onduty = duty
end)

local function LoadPlayerUniform()
    Framework.Functions.TriggerCallback("fivem-appearance:server:getUniform", function(uniformData)
...
RegisterNetEvent('Framework:Client:OnPlayerLoaded', function()
    ...
    PlayerJob = PlayerData.job
    PlayerGang = PlayerData.gang
    Framework.Functions.TriggerCallback('fivem-appearance:server:getpp', function(nopp)
    if nopp then 
        TriggerEvent('fivem-appearance:client:openClothingShop',true)
    end
end)
    Framework.Functions.TriggerCallback('fivem-appearance:server:getAppearance', function(appearance)
...
RegisterNetEvent('qb-clothes:client:CreateFirstCharacter', function()
    ...
    SetEntityCoords(playerPed, -1298.140 + (math.random(-1000, 1000) / 100), 272.933 + (math.random(-1000, 1000) / 100), 63.575)
    FreezeEntityPosition(playerPed, true)
    SetEntityHeading(playerPed,150.88569)
    Framework.Functions.GetPlayerData(function(pd)
        ...
        Framework.Functions.TriggerCallback("Framework:HasPermission", function(permission)
        ...
        SetGameplayCamRelativeHeading(GetEntityHeading(PlayerPedId()) -180)
        Wait(1000)
...
function OpenShop(config, isPedMenu, shopType)
    Framework.Functions.TriggerCallback("fivem-appearance:server:hasMoney", function(hasMoney, money)
        if not hasMoney and not isPedMenu then
            Framework.Functions.Notify("Нямаш достатъчно пари в банката. Необходими $" .. money, "error")
            return
        end
        ...
        exports['fivem-appearance']:startPlayerCustomization(function(appearance)
            if appearance then
                ...
                Framework.Functions.Notify("Cancelled Customization")
            end
...
RegisterNetEvent('fivem-appearance:client:saveOutfit', function()
    local keyboard = exports["nocore-keyboard"]:KeyboardInput({
        header = "Наименувай си облеклото",
        rows = { { id = 0, label = "Име", helper = "" } }
    })

    if keyboard and keyboard[1] then
        Wait(500)
        Framework.Functions.TriggerCallback("fivem-appearance:server:getOutfits", function(outfits)
            local outfitExists = false
            for i = 1, #outfits, 1 do
                if outfits[i].outfitname == keyboard[1].input then
                    outfitExists = true
                    break
                end
            end
            ...
            if outfitExists then
                Framework.Functions.Notify("Вече съществува облекло с това име.", "error")
                return
            end
            ...
            TriggerServerEvent('fivem-appearance:server:saveOutfit', keyboard[1].input, pedModel, pedComponents, pedProps)
        end)
    end
end)
...
function OpenMenu(isPedMenu, backEvent, menuType, menuData)
    ...
    local outfitMenuItems = {{
        header = "Смени облекло",
        txt = "Избери си някое от запазените ти облекла",
        icon = 'fas fa-shirt',
        params = {
        ...
        header = "Запази ново облекло",
        txt = "Запази ново облекло, което можеш да използваш по-късно",
        icon = 'fa-solid fa-shirt',
        params = {
            event = "fivem-appearance:client:saveOutfit"
        }
    }, {
        header = "Изтрий облекло",
        txt = "Не ти харесва облекло? Ми изтрий го",
    ...
    if menuType == "default" then
        local header = "Закупи дрехи - $" .. Config.ClothingCost
        if isPedMenu then
            header = "Смени дрехи"
        end
        menuItems[#menuItems + 1] = {
            header = "Магазин за дрехи",
            icon = "fas fa-shirt",
            isMenuHeader = true -- Set to true to make a nonclickable title
        }
        menuItems[#menuItems + 1] = {
            header = header,
            txt = "Изберете от широка гама артикули за носене",
    ...
    exports['nocore-context']:openMenu(menuItems)
end
...
RegisterNetEvent("fivem-appearance:client:changeOutfitMenu", function(data)
    Framework.Functions.TriggerCallback('fivem-appearance:server:getOutfits', function(result)
        local outfitMenu = {{
            header = 'Назад',
            icon = 'fa-solid fa-angle-left',
            params = {
        ...
        exports['nocore-context']:openMenu(outfitMenu)
    end)
end)

RegisterNetEvent("fivem-appearance:client:changeOutfitMenuInHousing", function(data)
    Framework.Functions.TriggerCallback('fivem-appearance:server:getOutfits', function(result)
        local outfitMenu = {}
        for i = 1, #result, 1 do
            outfitMenu[#outfitMenu + 1] = {
    ...
    if pedModel ~= data.model then
        Framework.Functions.TriggerCallback("fivem-appearance:server:getAppearance", function(appearance)
            if appearance then
                ...
                Framework.Functions.Notify("Нещо се е объркало. Облеклото, с което се опитвате да се преоблечете, няма базов вид.", "error")
                failed = true
            end
        end, data.model)
    else
...
RegisterNetEvent("fivem-appearance:client:deleteOutfitMenu", function(data)
    Framework.Functions.TriggerCallback('fivem-appearance:server:getOutfits', function(result)
        ...
        for i = 1, #result, 1 do
            outfitMenu[#outfitMenu + 1] = {
                header = 'Изтрий "' .. result[i].outfitname .. '"',
                txt = 'Не можеш да го върнеш никога!',
                icon = 'fa-solid fa-trash-can',
                params = {
...
RegisterNetEvent('fivem-appearance:client:deleteOutfit', function(id)
    ...
    Framework.Functions.Notify('Облеклото е изтрито успешно', 'error')
end)
...
RegisterNetEvent('fivem-appearance:client:reloadSkin', function()
    ...
    if CheckPlayerMeta() or IsPedInAnyVehicle(playerPed, true) or IsPedFalling(playerPed) then
        Framework.Functions.Notify("Не можеш да го използваш в момента", "error", 5000) 
        return
    end
    TriggerEvent("backitems:displayItems", false)
    ...
    clothingCombo:onPlayerInOut(function(isPointInside, _, zone)
        if isPointInside then
            ...
            if zoneName == 'clothing' then
                exports['okokTextUI']:Open('[E] Магазин за дрехи', 'purple', 'left')
            elseif zoneName == 'barber' then
                exports['okokTextUI']:Open('[E] Бръснарница', 'purple', 'left')
            elseif zoneName == 'tattoo' then
                exports['okokTextUI']:Open('[E] Татуировки', 'purple', 'left')
            -- elseif zoneName == 'surgeon' then
                -- exports['okokTextUI']:Open('[E] Пластична операция', 'purple', 'left')
            end
        else
            inZone = false
    ...
    clothingRoomsCombo:onPlayerInOut(function(isPointInside, _, zone)
        if isPointInside then
            ...
            if jobName == clothingRoom.requiredJob then
                if CheckDuty() then
                    inZone = true
                    exports['okokTextUI']:Open('[E] Clothing Room', 'purple', 'left')
                end
            end
        else
            inZone = false
...
local function SetupPlayerOutfitRoomZones()
    ...
    playerOutfitRoomsCombo:onPlayerInOut(function(isPointInside, _, zone)
        if isPointInside then
            ...
            if isAllowed then
                inZone = true
                exports['okokTextUI']:Open('[E] Облекла', 'purple', 'left')
            end
        else
            inZone = false
            exports['okokTextUI']:Close()
...
local function SetupStoreTargets()
    for k, v in pairs(Config.Stores) do
        ...
        if Config.EnablePedsForShops then
            ...
            exports['nocore-eye']:AddTargetEntity(TargetPeds.Store[k], parameters)
        else
            exports['nocore-eye']:AddBoxZone(v.shopType .. k, v.coords, v.length, v.width, {
                name = v.shopType .. k,
                debugPoly = Config.Debug,
                minZ = v.coords.z - 1,
                maxZ = v.coords.z + 1,
...
local function SetupClothingRoomTargets()
    for k, v in pairs(Config.ClothingRooms) do
        ...
        if Config.EnablePedsForClothingRooms then
            ...
            exports['nocore-eye']:AddTargetEntity(TargetPeds.ClothingRoom[k], parameters)
        else
            exports['nocore-eye']:AddBoxZone('clothing_' .. v.requiredJob .. k, v.coords, v.length, v.width, {
...
local function SetupPlayerOutfitRoomTargets()
    for k, v in pairs(Config.PlayerOutfitRooms) do
        ...
        if Config.EnablePedsForClothingRooms then
            ...
            exports['nocore-eye']:AddBoxZone('playeroutfitroom_' .. k, v.coords, v.length, v.width, {
...
--[[
RegisterCommand("pedid", function()
    local ped = PlayerPedId()
    print(GetHashKey(ped))
end)
]]
