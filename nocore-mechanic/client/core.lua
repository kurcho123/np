        ...
        if (not uiOpen) then
            if ((playerVeh ~= 0) and (DoesEntityExist(playerVeh)) and (GetPedInVehicleSeat(playerVeh, -1) == playerPed)) then
                ...
                for i = 1, #Config.Positions, 1 do
                    ...
                    if (not tempPos.whitelistJobName or (jobName == tempPos.whitelistJobName and jobGrade >= tempPos.MinGrade)) then
                        ...
                        if (tempDist <= tempActionDist) then
                            ...
                            if (not IsHudHidden()) then
                                DrawHelpText('Натисни ~' .. Config.Keys.action.name ..
                                                 '~ за да тунинговаш', true)
                            end
...
function closeUI(sendToUI, resetVehToDefault)
    ...
    TriggerServerEvent('mechanic:sv:saveVehicleProps', Framework.Functions.GetVehicleProperties(customVehicle), exports['nocore-fakeplate']:GetRealPlate(GetVehicleNumberPlateText(customVehicle)))
...
function updateMenu(menuId)
    ...
    local whitelistJobName = nil
    if (customConfigPosIndex and customConfigPosIndex > 0 and customConfigPosIndex <= #Config.Positions and
        Config.Positions[customConfigPosIndex]) then
        whitelistJobName = Config.Positions[customConfigPosIndex].whitelistJobName
    end
...
function optionsShouldShow(menu)
    ...
    for i = 1, #menu.options, 1 do
        ...
        if (shouldShow and menu.options[i].openSubMenu ~= nil) then
            ...
            for i = 1, #subMenu.options, 1 do
                if (subMenu.options[i].modType ~= nil) then
                    if (GetNumVehicleModData(customVehicle, subMenu.options[i].modType) >= 0 or
                        subMenu.options[i].openSubMenu ~= nil) then
                        tempShouldShow = true
                        break
                    end
                end
        ...
        if (not isOpenByAdmin) then
            if (customConfigPosIndex and customConfigPosIndex > 0 and customConfigPosIndex <= #Config.Positions and
                Config.Positions[customConfigPosIndex] and jobName ~=
                Config.Positions[customConfigPosIndex].whitelistJobName) then
            ...
            tempOption.onSelect = function()
                ...
                Config.Menus['extras_on_off'] = {
                    ...
                    options = {{
                        ...
                        price = tempPrice,
                        onHover = function()
                            SetVehicleModData(customVehicle, menuOption.modType, {
                                id = (i + 1),
                                enable = 1
                            })
                        end,
