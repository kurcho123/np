...
Citizen.CreateThread(function()
	for k, v in pairs(Config.Chairs) do
		Chairs["CatChair"..k] =
		exports['nocore-eye']:AddBoxZone("CatChair"..k, v.location, v.width, v.height, { name="CatChair"..k, heading = v.heading, debugPoly=Config.Debug, minZ = v.minZ, maxZ = v.maxZ, }, 
			{ options = { { event = "nocore-catcafe:Chair", icon = "fas fa-chair", label = "Седни", loc = v.location, head = v.heading, seat = v.seat }, },
			  distance = v.distance
		})
	end
end)

RegisterNetEvent('nocore-catcafe:Chair', function(data)
	local canSit = true
	local sitting = false
	for k, v in pairs(Framework.Functions.GetPlayersFromCoords(data.loc, 0.6)) do
		...
		if dist <= 0.4 then TriggerEvent("Framework:Notify", "Someone is already sitting there..") canSit = false end
	end
	if canSit then
...
AddEventHandler('onResourceStop', function(r) if r == GetCurrentResourceName() then for k, v in pairs(Chairs) do exports['nocore-eye']:RemoveZone(k) end end end)