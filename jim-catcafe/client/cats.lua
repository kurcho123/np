...
Citizen.CreateThread(function()
	for k, v in pairs(Config.PedList) do
		if v.clickable then
			exports['nocore-eye']:AddBoxZone("CatPat"..k, vector3(v.coords.x, v.coords.y, v.coords.z-0.3), 0.7, 0.7, { name="CatPat"..k, heading = 0, debugPoly=false, minZ= v.coords.z-1.5, maxZ=v.coords.z-0.5 }, 
				{ options = { { event = "nocore-catcafe:CatPat", icon = "fas fa-paw", label = "Гали котката" }, },
				  distance = 1.5
			})
		end
	end
...
RegisterNetEvent('nocore-catcafe:CatPat')
AddEventHandler('nocore-catcafe:CatPat', function()
