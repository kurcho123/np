...
local Drawables = {
	...
	["Shoes"] = {
		Drawable = 6,
		Table = {Standalone = true, Male = 96, Female = 106},
...
local Extras = {
	...
	["Pants"] = {
		Drawable = 4,
		Table = {Standalone = true, Male = 127, Female = 15},
...
--[[ RegisterCommand("box", function ( ... )
    local localpedmodel = GetEntityModel(PlayerPedId())
    if HaveBagOnHead == false then
		if localpedmodel ~= GetHashKey('mp_f_freemode_01') and localpedmodel ~= GetHashKey('mp_f_freemode_02') then
    local playerPed = PlayerPedId()
    Bag = CreateObject(GetHashKey("v_serv_abox_04"), 0, 0, 0, true, true, true) -- Create head bag object!
