...
local function GetBuffs()
    ...
    Wait(1)
    local p = promise.new()
    Framework.Functions.TriggerCallback('buffs:server:fetchBuffs', function(result)
        p:resolve(result)
    end)
...
local function AddBuff(buffName, time)
    local p = promise.new()
    Framework.Functions.TriggerCallback('buffs:server:addBuff', function(result)
        p:resolve(result)
    end, buffName, time)
    return Citizen.Await(p)
...
local function StaminaBuffEffect(time, value)
    ...
    if not hasStaminaBuffActive then 
        ...
        CreateThread(function()
            ...
            while exports['nocore-buffs']:HasBuff("stamina") do
                Wait(500)
...
local function SwimmingBuffEffect(time, value)
    ...
    if not hasSwimmingBuffActive then 
        ...
        CreateThread(function()
            ...
            while exports['nocore-buffs']:HasBuff("swimming") do
                Wait(500)
                SetPlayerStamina(PlayerId(), GetPlayerStamina(PlayerId()) + math.random(1,10))
            end
