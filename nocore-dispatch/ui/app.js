...
$(document).ready(() => {
    window.addEventListener('message', function (event) {
        ...
        if (data.update == "newCall") {
            addNewCall(data.callID, data.timer, data.data, data.isPolice, false);
        };
        if (data.update == "getCalls") {
            addNewCall(data.callID, data.timer, data.data, data.isPolice, true);
        };
    });
});
...
function addNewCall(callID, timer, info, isPolice, silent) {
    ...
    if (info['firstColor'] && info['secondColor']) {
        DispatchItem += `<div class="call-bottom-info"><span class="fas fa-spray-can"></span>${info['firstColor']} ON ${info['secondColor']}</div>`
    } else if (info['firstColor'])  {
