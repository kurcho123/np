...
$(document).on('click', '.close-menu > i', function(e) {
  e.preventDefault();
  $.post(`http://nocore-garages/Click`, JSON.stringify({}));
  CloseGarage()
});
...
$(document).on('click', '.takeout', function(e) {
  ...
  if (CurrentMenu != null && CurrentMenu != undefined) {
    if (CurrentVehicleData != null && CurrentVehicleData != undefined) {
      CloseGarage()
      $.post(`http://nocore-garages/TakeOutVehicle`, JSON.stringify({Plate: CurrentVehicleData.Plate, Model: CurrentVehicleData.Model, State: CurrentVehicleData.State, Engine: CurrentVehicleData.Engine, Body: CurrentVehicleData.Body, Fuel: CurrentVehicleData.Fuel, Price: CurrentVehicleData.Price}));
    }
  }
});
...
SetupGarageVehicles = function(data) {
  ...
  for (const [key, value] of Object.entries(data.garagevehicles)) {
    if (value['State'] === 'out') {
      var AddVehicle = '<div class="vecicle-container" data-plate='+value['Plate']+' data-model='+value['Model']+' data-state='+value['State']+' data-fuel='+value['Fuel']+' data-motor='+value['Motor']+' data-body='+value['Body']+'><span class="vehicle-status status-out">Извън гараж</span><p>'+ value['Name'] +'  <i class="fas fa-car"></i></p></div>'
      $('.menu-container').append(AddVehicle);
    } else {
      var AddVehicle = '<div class="vecicle-container" data-plate='+value['Plate']+' data-model='+value['Model']+' data-state='+value['State']+' data-fuel='+value['Fuel']+' data-motor='+value['Motor']+' data-body='+value['Body']+'><span class="vehicle-status status-in">В гараж</span><p>'+ value['Name'] +'  <i class="fas fa-car"></i></p></div>'
      $('.menu-container').append(AddVehicle);
    }
  }
...
CloseGarage = function() {
  ...
  $.post(`http://nocore-garages/CloseNui`, JSON.stringify({}));
}
