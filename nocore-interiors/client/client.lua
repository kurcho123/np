...
function HouseTierOne(spawn)
    ...
    Citizen.Wait(100)
    SetEntityCoords(PlayerPedId(), spawn.x - POIOffsets.exit.x , spawn.y - POIOffsets.exit.y, spawn.z - POIOffsets.exit.x)
    Citizen.Wait(45)
    DoScreenFadeIn(500)
    return {objects, POIOffsets}
end
...
function GarageTierOne(spawn)
    local objects = {}
    local POIOffsets = {}
    POIOffsets.exit = json.decode('{"z":33.9,"y":1.5,"x":-2.5,"h":0.0}')
    DoScreenFadeOut(500)
    while not IsScreenFadedOut() do
        Citizen.Wait(10)
    end
...
function OfficeOne(spawn)
    local objects = {}
    local POIOffsets = {}
	POIOffsets.exit = json.decode('{"z":32.8,"y":-4.5,"x":-1.0,"h":0.0}')
    DoScreenFadeOut(500)
    while not IsScreenFadedOut() do
        Citizen.Wait(10)
    end
    ...
    local shell = CreateObject(`shell_office1`, spawn.x, spawn.y, spawn.z, false, false, false)
    FreezeEntityPosition(shell, true)
    table.insert(objects, shell)
    Citizen.Wait(100)
    SetEntityCoords(PlayerPedId(), spawn.x - POIOffsets.exit.x , spawn.y - POIOffsets.exit.y, spawn.z + 1.5)
...
function OfficeTwo(spawn)
    local objects = {}
    local POIOffsets = {}
	POIOffsets.exit = json.decode('{"z":33.8,"y":2.0,"x":-3.7,"h":0.0}')
    DoScreenFadeOut(500)
    while not IsScreenFadedOut() do
        Citizen.Wait(10)
    end
    ...
    local shell = CreateObject(`shell_office2`, spawn.x, spawn.y, spawn.z, false, false, false)
    FreezeEntityPosition(shell, true)
    table.insert(objects, shell)
    Citizen.Wait(100)
    SetEntityCoords(PlayerPedId(), spawn.x - POIOffsets.exit.x , spawn.y - POIOffsets.exit.y, spawn.z + 1.5)
...
function OfficeBig(spawn)
    local objects = {}
    local POIOffsets = {}
	POIOffsets.exit = json.decode('{"z":30.0,"y":2.8,"x":16.15,"h":0.0}')
    DoScreenFadeOut(500)
    while not IsScreenFadedOut() do
        Citizen.Wait(10)
    end
    ...
    table.insert(objects, shell)
    Citizen.Wait(100)
    SetEntityCoords(PlayerPedId(), spawn.x - POIOffsets.exit.x , spawn.y - POIOffsets.exit.y, spawn.z + 4.0)
    Citizen.Wait(45)
    DoScreenFadeIn(500)
    return {objects, POIOffsets}
end
...
function HouseRobTierOne(spawn)
	...
	local table3 = CreateObject(GetHashKey("V_Res_M_DineTble_replace"), spawn.x-3.50712600, spawn.y-4.13621600, spawn.z+1.29625800, false, false, false)
	table.insert(objects, table3)
...
--[[ function CreateObject(modelHash, x, y, z, isNetwork, thisScriptCheck, dynamic)
	local CreateObject(modelHash, x, y, z, isNetwork, thisScriptCheck, dynamic)
end ]]