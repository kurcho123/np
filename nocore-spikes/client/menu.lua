if Config.Menu.Enabled then
    CreateThread(function()
        ...
        if Config.Menu.Command then
            RegisterCommand(Config.Menu.Command, function()
                if isPolice or not Config.RequireJobRemove then
                    TriggerEvent("nocore-spikes:spikestripMenu")
                else
        ...
        if Config.Framework == "esx" and not Config.FrameworkFeatures.UseWarmenu then
            ...
            RegisterNetEvent("nocore-spikes:spikestripMenu")
            AddEventHandler("nocore-spikes:spikestripMenu", function()
                if isPolice or not Config.RequireJobRemove then
                    ...
                    local elements = {
                        {
                            ...
                            params = {
                                event = "nocore-spikes:placeSpikestrip",
                            }
                        }
                    }
                    if isPolice or not Config.RequireJobRemove then
                        table.insert(elements, {
                            ...
                            params = {
                                event = "nocore-spikes:removeSpikestrip",
                            }
                        })
                    end
                    table.insert(elements, {
                        ...
                        params = {
                            event = "nocore-context:client:closeMenu",
                        }
                    })
                    exports["nocore-context"]:openMenu(elements)
                else
