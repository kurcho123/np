...
RegisterNetEvent('nocore-hospital:client:bed')
AddEventHandler('nocore-hospital:client:bed', function()
	if Framework.Functions.GetPlayerData().job.name == 'ambulance' then
		LoadModel('prop_ld_binbag_01')
		local brancard = CreateObject(GetHashKey('prop_ld_binbag_01'), GetEntityCoords(PlayerPedId()), true)
	end
...
RegisterNetEvent('nocore-hospital:client:bed:del')
AddEventHandler('nocore-hospital:client:bed:del', function()
	local brancard = GetClosestObjectOfType(GetEntityCoords(PlayerPedId()), 8.0, GetHashKey('prop_ld_binbag_01'))
	if DoesEntityExist(brancard) then
		DeleteEntity(brancard)
	end
	...
	while true do
		...
		if DoesEntityExist(closestObject) then
			...
			if GetDistanceBetweenCoords(pedCoords, sitCoords, true) <= 2.0 then
				Framework.Functions.DrawText3D(sitCoords.x, sitCoords.y, sitCoords.z, _U("laydown"), 0.4)
			...
			if GetDistanceBetweenCoords(pedCoords, pickupCoords, true) <= 2.5 then
				Framework.Functions.DrawText3D(pickupCoords.x, pickupCoords.y, pickupCoords.z , _U("push"), 0.4)
...
Sit2 = function(brancardObject)
	...
	if IsEntityAttachedToEntity(brancardObject, PlayerPedId()) then
		DetachEntity(brancardObject, true, true)
	end

	AttachEntityToEntity(PlayerPedId(), brancardObject, 0, 0, 0.0, 1.11, 0.0, 0.0, 180.0, 0.0, false, false, false, false, 2, true)
...
PickUp2 = function(brancardObject)
	...
	AttachEntityToEntity(brancardObject, PlayerPedId(), GetPedBoneIndex(PlayerPedId(),  28422), -0.00, -1.1, -0.44, 195.0, 180.0, 180.0, 0.0, false, false, true, false, 2, true)
