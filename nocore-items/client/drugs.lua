...
RegisterNetEvent('nocore-items:client:use:joint')
AddEventHandler('nocore-items:client:use:joint', function()
  Citizen.SetTimeout(1000, function()
    Framework.Functions.Progressbar("smoke-joint", "Гърмиш 120ката..", 2500, false, true, {
     ...
     disableInventory = true,
        ...
        TriggerEvent("nocore-inventory:client:ItemBox", Framework.Shared.Items["joint"], "remove")
        TriggerEvent('nocore-items:client:joint:effect')
        ...
        TriggerEvent("nocore-police:client:SetStatus", "weedsmell", 200)
      end, function() -- Cancel
...
RegisterNetEvent("nocore-items:client:use:coke")
AddEventHandler("nocore-items:client:use:coke", function()
     Framework.Functions.Progressbar("snort_coke", "Пукаш бело", math.random(5000, 8000), false, true, {
         disableMovement = false,
         disableCarMovement = false,
         disableMouse = false,
         disableCombat = true,
         ...
         TriggerEvent("nocore-inventory:client:ItemBox", Framework.Shared.Items["coke-bag"], "remove")
         TriggerEvent("nocore-police:client:SetStatus", "widepupils", 200)
         TriggerServerEvent('hud:server:RelieveStress', math.random(2, 6))
         CokeBagEffect()
     end, function() -- Cancel
         StopAnimTask(PlayerPedId(), "anim@amb@nightclub@peds@", "missfbi3_party_snort_coke_b_male3", 1.0)
...
RegisterNetEvent("nocore-items:client:use:lsd")
AddEventHandler("nocore-items:client:use:lsd", function()
  Citizen.SetTimeout(1000, function()
    Framework.Functions.Progressbar("snort_coke", "Пукаш бело..", 3000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
       ...
       flags = 49,
        ...
        TriggerEvent("nocore-inventory:client:ItemBox", Framework.Shared.Items["lsd-strip"], "remove")
        TriggerEvent("nocore-police:client:SetStatus", "widepupils", 200)
        TriggerServerEvent('hud:server:RelieveStress', math.random(2, 6))
        Effectlsd()
    end, function() -- Cancel
...
RegisterNetEvent('nocore-items:client:joint:effect')
AddEventHandler('nocore-items:client:joint:effect', function()
  OnWeed = true
  while OnWeed do
    Citizen.Wait(1)
    if WeedTime > 0 then
...
function Effectlsd()
  ...
  AnimpostfxPlay("DrugsDrivingIn", 20000, 0)
  Wait(10000)
