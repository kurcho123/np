...
RegisterNetEvent('nocore-admin:client:ToggleNoClip', function()
    toggleNoClipMode()
end)
...
function toggleNoClipMode(forceMode)
	if forceMode ~= nil then
		...
		inputRotEnabled = noClipEnabled
        TriggerServerEvent('nocore-logs:server:SendLog', 'admin', 'Admin menu', 'pink', string.format("**%s** (CitizenID: %s | ID: %s) - Set NoClip to **%s**",
