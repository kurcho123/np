...
CreateThread(function()
    while true do
        ...
        if NetCheck1 or NetCheck2 then
            TriggerServerEvent('nocore-admin:server:GetPlayersForBlips')
        end
    end
end)

RegisterNetEvent('nocore-admin:client:toggleBlips', function()
    TriggerServerEvent('nocore-admin:server:check')
    if not ShowBlips then
        ShowBlips = true
        NetCheck1 = true
        Framework.Functions.Notify(Lang:t("success.blips_activated"), "success")
    else
        ShowBlips = false
        Framework.Functions.Notify(Lang:t("error.blips_deactivated"), "error")
    end
    TriggerServerEvent('nocore-logs:server:SendLog', 'admin', 'Admin menu', 'pink', string.format("**%s** (CitizenID: %s | ID: %s) - Set ShowBlips to **%s**",
    GetPlayerName(PlayerId()), Admin.citizenid, Admin.source, ShowBlips))
end)

RegisterNetEvent('nocore-admin:client:toggleNames', function()
    TriggerServerEvent('nocore-admin:server:check')
    if not ShowNames then
        ShowNames = true
        NetCheck2 = true
        Framework.Functions.Notify(Lang:t("success.names_activated"), "success")
    else
        ShowNames = false
        Framework.Functions.Notify(Lang:t("error.names_deactivated"), "error")
    end
    TriggerServerEvent('nocore-logs:server:SendLog', 'admin', 'Admin menu', 'pink', string.format("**%s** (CitizenID: %s | ID: %s) - Set ToggleNames to **%s**",
    GetPlayerName(PlayerId()), Admin.citizenid, Admin.source, ShowNames))
end)

RegisterNetEvent('nocore-admin:client:Show', function(players)
