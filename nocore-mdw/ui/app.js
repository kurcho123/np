...
function SaveIncident() {
    if (canSaveProfile == true) {
        ...
        $('.manage-incidents-weapons-holder').find('div').each(function() {
            if ($(this).text() != "") { weapons.push($(this).text()) }
        });
        ...
        let associatedVehicles = new Array()

        $(".associated-vehicles-incidents-tags-holder").children('div').each(function(index) {
            associatedVehicles.push({
                plate: $(this).data("id"),
            })
        });
        ...
        $(".associated-incidents-user-container").each(function(index) {
            ...
            var issentenced = false
            var charges = new Array()

            $(".associated-incidents-user-tags-holder").children('div').each(function(index) {
                if ($(this).data("id") == cid) {
                    if ($(this).hasClass("green-tag")) {
                        if ($(this).text() == 'Виновен') { guilty = true }
                        if ($(this).text() == 'Издирван') { warrant = true }
                        if ($(this).text() == 'Разследван') { processed = true }
                        if ($(this).text() == 'Замесен') { isassociated = true }
                    }
                }
            });
            ...
            if ($('.associated-incidents-user-sentence-btn').filter("[data-id='" + $(this).data("id") + "']").hasClass("green-tag")) {
                issentenced = true
            }
...
function OpenCamera(id) {
    ...
    $.post(`https://${GetParentResourceName()}/OpenCam`, JSON.stringify({
        camid: id
    }));
    return;
}
...
function Setlocation(x, y) {
    //this.closeMDT()
    $.post(`https://${GetParentResourceName()}/SetLocation`, JSON.stringify({
        x: x,
        y: y
    }));
    return;
...
$(document).ready(() => {
    ...
    $(".weaponsearch-items-c").on("click", ".profile-item", function() {
        let id = $(this).data("id");
        $.post(`https://${GetParentResourceName()}/getProfileData`, JSON.stringify({
            id: id
        }));
    ...
    $(".weaponsearch-items-i").on("click", ".incidents-item", function() {
    ...
    $(".associated-vehicles-incidents-tags-add-btn").on("click", "", function() {
        VehSearchGoesTo = "Associated"
        document.addEventListener('mouseup', onMouseDownIcidents);
        $(".icidents-vehicle-search-container").fadeIn(250)
        $(".close-all").css("filter", "brightness(15%)");
    });

    $(".manage-incidents-officers-add-btn").on("click", "", function() {
        SearchGoesTo = "Officers"
        document.addEventListener('mouseup', onMouseDownIcidents);
        $(".icidents-person-search-container").fadeIn(250)
        $(".close-all").css("filter", "brightness(15%)");
    });

    $(".manage-incidents-civilians-add-btn").on("click", "", function() {
        SearchGoesTo = "Civilians"
        document.addEventListener('mouseup', onMouseDownIcidents);
        $(".icidents-person-search-container").fadeIn(250)
        $(".close-all").css("filter", "brightness(15%)");
    });
    ...
    $(".manage-profile-save").click(function() {
        if (canSaveProfile == true) {
            ...
            setTimeout(() => {
                ...
                if (!CheckURL(newpfp) & !ImageCheck(newpfp)) {
    ...
    $(".manage-incidents-title-holder").on("click", ".manage-incidents-create", function() {
        let tempalte = "Код на сигнала и локация:\n\nРапорт:\n\nДоказателства:\n\nКонфискувани незаконни или незаконно придобити вещи от задържан(ите): \n1. Две имена: /конфискувани вещи/\n2. Две имена: /конфискувани вещи/\n3. Две имена: /конфискувани вещи/\n4. Две имена: /конфискувани вещи/\n\nКонфискувани вещи, които са пратени в /jailitems id/:\n\nЗадържаният(те) пледира(т): \n1. Две имена: /виновен, невинен, няма възражения/\n2. Две имена: /виновен, невинен, няма възражения/\n3. Две имена: /виновен, невинен, няма възражения/\n4. Две имена: /виновен, невинен, няма възражения/\n\nАвтор на доклада:"
        $("#manage-incidents-title-input").val("Име - " + $(".date").html() +" - "+ $(".time").html());
        $(".manage-incidents-reports-content").val(tempalte);

        $(".manage-incidents-tags-holder").empty();
        $(".manage-incidents-weapons-holder").empty();
        ...
        $(".associated-vehicles-incidents-tags-holder").empty();
        $(".manage-incidents-title-holder").prepend(
            `
            <div class="manage-incidents-title">Менажиране на Доклад</div>
        ...
        $(".manage-incidents-editing-title").html("В момента създавате нов доклад");
        $(".manage-incidents-editing-title").data("id", 0);

        $(".manage-incidents-tags-add-btn").css("pointer-events", "auto")
        $(".manage-incidents-weapons-add-btn").css("pointer-events", "auto")
        ...
        $(".associated-vehicles-incidents-tags-add-btn").css("pointer-events", "auto")
    ...
    $(".weapons-add-btn").click(function() {
        if ($(".manage-profile-citizenid-input").val()) {
            if (canInputWeapon) {
                ...
                canInputWeapon = false
            } else if (!canInputWeapon) {
                $(this).removeClass('fa-minus').addClass('fa-plus');
                $(".weapon-input").remove()
                canInputWeapon = true
            }
    ...
    $(".profile-incidents-holder").on("contextmenu", ".profile-incidents-incident", function(e) {
        let args = [{
    ...
    $(".weaponsearch-title").click(function() {
        if (canSearchForProfiles == true) {
            if ($(".weaponsearch-search-input").css("display") == "none") {
                $(".weaponsearch-search-input").slideDown(250);
                $(".weaponsearch-search-input").css("display", "block")
            } else {
                $(".weaponsearch-search-input").slideUp(250);
                setTimeout(() => {
                    $(".weaponsearch-search-input").css("display", "none")
                }, 250);
            }
        }
    });

    $('#weaponsearch-search-input').keydown(function(e) {
        if (e.keyCode === 13 && canSearchForProfiles == true) {
            let WeaponID = $("#weaponsearch-search-input").val()
            if (WeaponID !== "") {
                canSearchForProfiles = false
                $.post(`https://${GetParentResourceName()}/searchWeapons`, JSON.stringify({
                    WeaponID: WeaponID
                }));
                $(".weaponsearch-items").empty();
                $('.weaponsearch-items').prepend(`<div class="profile-loader"></div>`);
            }
        }
    });
    ...
    $(".manage-incidents-officers-holder").on("click", ".manage-incidents-officers", function() {
        const name = $(this).text();
        fidgetSpinner(".profile-page-container");
        currentTab = ".profile-page-container";
        setTimeout(() => {
    ...
    $(".associated-incidents-tags-holder").on("click", ".associated-incidents-tag", function() {
        const name = $(this).text();
        fidgetSpinner(".profile-page-container");
        currentTab = ".profile-page-container";
        setTimeout(() => {
    ...
    document.onkeyup = function(data) {
        if (data.which == 27) {
            ...
            if ($(".respond-calls-container").css("display") == "flex") {
                console.log('respond-calls-container')
                shouldClose = false
                $(".respond-calls-container").fadeOut(500)
                setTimeout(() => { $(".close-all").css("filter", "none"); }, 250);
            }

            if ($(".gallery-image-div").css("display") == "block") {
                console.log('gallery-image-div')
                shouldClose = false
                $(".gallery-image-div").fadeOut(150);
                $(".gallery-image-div").css("display", "none");
                $(".close-all").css("filter", "none");
            }

            if ($(".incidents-image-enlarged").css("display") == "block") {
                console.log('incidents-image-enlarged')
                shouldClose = false
            ...
            if ($(".icidents-person-search-container").css("display") != "none") {
                console.log('icidents-person-search-container')
                shouldClose = false
                $(".icidents-person-search-container").fadeOut(250);
                $(".close-all").css("filter", "none");
            }

            if ($(".icidents-vehicle-search-container").css("display") != "none") {
                console.log('icidents-vehicle-search-container')
                shouldClose = false
                $(".icidents-vehicle-search-container").fadeOut(250);
                $(".close-all").css("filter", "none");
            }

            if ($(".incidents-charges-table").css("display") != "none") {
                console.log('incidents-charges-table')
                shouldClose = false
            ...
            if ($(".dispatch-attached-units").css("display") != "none") {
                console.log('dispatch-attached-units')
                shouldClose = false
    ...
    $(".manage-incidents-weapons-add-btn").click(function() {
        if ($(".weapon-incident-input")[0]) {
            $(this).removeClass('fa-minus').addClass('fa-plus');
            $(".weapon-incident-input").remove()
        } else {
            $(this).removeClass('fa-plus').addClass('fa-minus');
            $('.manage-incidents-weapons-holder').prepend(`<span contenteditable="true" class="weapon-incident-input"></span>`);
        }
    });
    ...
    $(".icidents-vehicle-search-name-input").on("keydown", "", function(e) {
        if (e.keyCode === 13) {
            let plate = $(".icidents-vehicle-search-name-input").val();
            $.post(`https://${GetParentResourceName()}/incidentSearchVehicle`, JSON.stringify({
                plate: plate
            }));
        }
    });
    ...
    $(".manage-incidents-weapons-holder").on("keydown", ".weapon-incident-input", function(e) {
        if (e.keyCode === 13) {
            $('.manage-incidents-weapons-holder').prepend(`<div class="manage-incidents-weapon tag">${$(".weapon-incident-input").text()}</div>`);
            // Have it save instantly if it's an existing report.
            if ($(".manage-incidents-weapons-add-btn").hasClass("fa-minus")) {
                $(".manage-incidents-weapons-add-btn").removeClass('fa-minus').addClass('fa-plus');
            }
            $(".weapon-incident-input").remove()
        }
    });
...
/*     $(".manage-incidents-civilians-add-btn").click(function() {
    ...
    $(".manage-bolos-title-holder").on("click", ".manage-bolos-new", function() {
        ...
        $(".manage-bolos-editing-title").html("Създавате ново издирване на МПС")
    ...
    $(".manage-bolos-title-holder").on("click", ".manage-bolos-save", function() {
        let existing = !($(".manage-bolos-editing-title").html() == 'Създавате ново издирване на МПС')
    ...
    $(".warrants-items").on("contextmenu", ".bolo-item", function(e) {
        args = [
            {
                "className": "view-bolo",
                "icon": "fas fa-search",
    ...
    $(".contextmenu").on("click", ".view-bolo", function() {
        const data = $(this).data("info")
        fidgetSpinner(".bolos-page-container");
        currentTab = ".bolos-page-container";
        setTimeout(() => {
            $(".bolos-search-input").slideDown(250);
            $(".bolos-search-input").css("display", "block")
            setTimeout(() => {
                $("#bolos-search-input:text").val(data.toString());
                $.post(`https://${GetParentResourceName()}/searchBolos`, JSON.stringify({
                    searchVal: data
                }));
                $(".bolos-items").empty();
    ...
    $(".incidents-ghost-holder").on("click", ".associated-incidents-user-tag", function() {
        if ($(this).hasClass("red-tag")) {
            ...
            if ($(this).text() == 'Замесен') {
                ...
                $(".associated-incidents-user-sentence-btn").filter(`[data-id="${$(this).data("id")}"]`).css("display", "none")
            }
        } else {
            ...
            if ($(this).text() == 'Замесен') {
                ...
                $(".associated-incidents-user-sentence-btn").filter(`[data-id="${$(this).data("id")}"]`).fadeIn(100)
            }
        }
    });

    $(".incidents-ghost-holder").on("click", ".associated-incidents-user-sentence-btn", function() {
        const report_id = $(".manage-incidents-editing-title").data("id")
        if (report_id != 0) {
            if ($(this).hasClass("red-tag")) {
                $(this).removeClass("red-tag");
                $(this).addClass("green-tag");
                const fine = $(".fine-amount").filter(`[data-id="${$(this).data("id")}"]`).val()
                const sentence = $(".sentence-amount").filter(`[data-id="${$(this).data("id")}"]`).val()
                const char_id = $(this).data("id")
                if (fine > 0) {
                    console.log(fine);
                    $.post(`https://${GetParentResourceName()}/sentencePlayer`, JSON.stringify({
                        fine: fine,
                        char_id: char_id,
                        reportid: report_id
                    }));
    ...
    $(".icidents-person-search-holder").on("click", ".icidents-person-search-item", function() {
        if (SearchGoesTo == "Associated") {
            ...
            $(".incidents-ghost-holder").prepend(
                `
                <div class="associated-incidents-user-container" data-id="${$(this).data("cid")}" data-name="${$(this).data("name")}">
                    <div class="associated-incidents-user-title">${$(this).data("info")}</div>
                    <div class="associated-incidents-user-tags-holder">
                        <div class="associated-incidents-user-tag red-tag" data-id="${$(this).data("cid")}">Издирван</div>
                        <div class="associated-incidents-user-tag red-tag" data-id="${$(this).data("cid")}">Виновен</div>
                        <div class="associated-incidents-user-tag red-tag" data-id="${$(this).data("cid")}">Разследван</div>
                        <div class="associated-incidents-user-tag red-tag" data-id="${$(this).data("cid")}">Замесен</div>
                    </div>
                    <div class="associated-incidents-user-holder" data-name="${$(this).data("cid")}">
                    </div>
                    <div class="manage-incidents-title-tag" data-id="${$(this).data("cid")}">Препоръчителна Глоба</div>
                    ...
                    <div class="manage-incidents-title-tag" data-id="${$(this).data("cid")}">Препоръчителна Присъда</div>
                    ...
                    <div class="manage-incidents-title-tag" data-id="${$(this).data("cid")}">Глоба</div>
                    ...
                    <div class="manage-incidents-title-tag" data-id="${$(this).data("cid")}">Присъда</div>
                    ...
                    <div class="associated-incidents-user-sentence-btn red-tag" data-id="${$(this).data("cid")}">Наложи Глоба</div>
                </div>
                `
            );
        } else if (SearchGoesTo == "Officers") {
            $(".icidents-person-search-container").fadeOut(250);
            $(".close-all").css("filter", "none");
            $('.manage-incidents-officers-holder').prepend(`<div class="manage-incidents-officers">${$(this).data("info")}</div>`);
        } else if (SearchGoesTo == "Civilians") {
            $(".icidents-person-search-container").fadeOut(250);
            $(".close-all").css("filter", "none");
            $('.manage-incidents-civilians-holder').prepend(`<div class="manage-incidents-civilians">${$(this).data("info")}</div>`);
        }

    });

    $(".icidents-vehicle-search-holder").on("click", ".icidents-vehicle-search-item", function() {
            $(".icidents-vehicle-search-container").fadeOut(250);
            $(".close-all").css("filter", "none");
            $(".associated-vehicles-incidents-tags-holder").prepend(
                `<div class="associated-vehicles-incidents-tag" data-id="${$(this).data("id")}">${$(this).data("id")}</div>`
            );

    });


    $(".associated-vehicles-incidents-tags-holder").on("contextmenu", ".associated-vehicles-incidents-tag", function(e) {
        let args = [{
            "className": "incidents-remove-vehicle",
            "icon": "fas fa-times",
            "text": "Премахни",
            "info": $(this).html(),
            "status": ""
    ...
    $(".contextmenu").on("click", ".incidents-remove-vehicle", function() {
        $(`.associated-vehicles-incidents-tag:contains(${$(this).data("info")})`).remove()
    });
    ...
    $(".contextmenu").on("click", ".incidents-remove-weapon", function() {
        $(`.tag:contains(${$(this).data("info")})`).remove()
    });

    $(".contextmenu").on("click", ".incidents-search-weapon", function() {
        const searchinfo = $(`.tag:contains(${$(this).data("info")})`).html()
        const serialnumber = searchinfo.split(" ")
        fidgetSpinner(".weaponsearch-page-container");
        currentTab = ".weaponsearch-page-container";
        setTimeout(() => {
            $(".weaponsearch-search-input").slideDown(250);
            $(".weaponsearch-search-input").css("display", "block")
            setTimeout(() => {
                $("#weaponsearch-search-input:text").val(serialnumber[0]);
                canSearchForProfiles = false
                $.post(`https://${GetParentResourceName()}/searchWeapons`, JSON.stringify({
                    WeaponID: serialnumber[0]
                }));
                $(".weaponsearch-items").empty();
                $('.weaponsearch-items').prepend(`<div class="profile-loader"></div>`);
            }, 250);
    ...
    $(".manage-incidents-weapons-holder").on("contextmenu", ".tag", function(e) {
        let args = [{
            "className": "incidents-remove-weapon",
            "icon": "fas fa-times",
            "text": "Премахни",
            "info": $(this).html(),
            "status": ""
        },{
            "className": "incidents-search-weapon",
            "icon": "fas fa-search",
            "text": "Потърси",
            "info": $(this).html(),
            "status": ""
    ...
    $(".contextmenu").on("click", ".incidents-remove-weapon-from-citizen", function() {
        $(`.weapon:contains(${$(this).data("info")})`).remove()
        let cid = $(".manage-profile-citizenid-input").val();
        if (cid) {
            $.post(`https://${GetParentResourceName()}/removeProfileWeapon`, JSON.stringify({
                cid: cid,
                text: $(this).data("info")
            }));
        }
    ...
    $(".weapons-holder").on("contextmenu", ".weapon", function(e) {
        let args = [{
            "className": "incidents-remove-weapon-from-citizen",
            "icon": "fas fa-times",
            "text": "Премахни",
            "info": $(this).html(),
            "status": ""
    ...
    $(".warrants-refresh").click(function() {
        $(".warrants-refresh").empty();
        $('.warrants-refresh').prepend(`<span class="fas fa-spinner fa-spin"></span>`);
        setTimeout(() => {
            $(".warrants-refresh").empty();
            $(".warrants-refresh").html("Refresh");
            canRefreshReports = true
            $.post(`https://${GetParentResourceName()}/refreshWarrants`, JSON.stringify({}));
        }, 1500);
    })
    ...
    $(".vehicle-information-title-holder").on("click", ".vehicle-information-save", function() {
        if (canSaveVehicle) {
            ...
            setTimeout(() => {
                ...
                if (!CheckURL(newImageurl) & !ImageCheck(newImageurl)) {
    ...
    function JobColors(sentJob) {
        if (sentJob) {
            PlayerJob = sentJob
            if (sentJob == 'police') {
                /* Set Colors, Badge, Title */
                document.documentElement.style.setProperty('--color-0', '#565656');
                document.documentElement.style.setProperty('--color-1', '#4e4e4e');
                document.documentElement.style.setProperty('--color-2', '#5e5f61');
                document.documentElement.style.setProperty('--color-3', '#7a7a7a');
                document.documentElement.style.setProperty('--color-4', '#585858');
                document.documentElement.style.setProperty('--color-5', '#343536');
                document.documentElement.style.setProperty('--color-6', '#2e2e2e');
                document.documentElement.style.setProperty('--color-7', 'rgb(70, 70, 70)');
                document.documentElement.style.setProperty('--color-8', '#9e9e9e');
                ...
                $(".bolo-nav-item").show();
                $(".dmv-nav-item").show();
                $(".weaponsearch-nav-item").show();
                $(".cam-nav-item").show();
                $(".codes-nav-item").show();
                $(".globi-nav-item").show();
                $(".npk-nav-item").show();
                $(".dispatch-title-ofsomesort").html("Разговор");
                ...
                document.documentElement.style.setProperty('--color-0', '#551e1e');
                ...
                $(".manage-profile-name-input-1").attr("readonly", true);
                $(".manage-profile-name-input-2").attr("readonly", true);
            } else if (sentJob == 'judge') {
                document.documentElement.style.setProperty('--color-0', '#553a1e');
    ...
    window.addEventListener('message', function(event) {
        ...
        if (e.type == "show") {
            ...
            let fishing = "red-tag"
            let bar = "red-tag"
            let business = "red-tag"
            let Licences = table['Licences']
            ...
            if (!canInputWeapon) {
                if ($(".weapons-add-btn").hasClass("fa-minus")) { $(".weapons-add-btn").removeClass('fa-minus').addClass('fa-plus'); }
                $(".weapon-input").remove()
                canInputWeapon = true
            }
            ...
            $(".manage-profile-editing-title").html("В момента редактирате " + table["firstname"] + " " + table["lastname"])
            $(".manage-profile-citizenid-input").val(table["citizenid"]);
            ...
            $(".manage-profile-blood-input").val(table["bloodtype"]);
            ...
            <div class="license-tag ${driver} Drivers">Drivers</div>
            ...
            $.each(table["weapons"], function(index, value) {
                $('.weapons-holder').prepend(`<div class="weapon">${value}</div>`);
            })
            $(".vehs-holder").empty();
            if (table["vehicles"]) {
            ...
            $(".properties-holder").empty();
            if (table["properties"]) { 
                $.each(table["properties"], function(index, value) {
                    $('.properties-holder').prepend(`<div class="white-tag">${value} </div>`);
                })
            }

            $(".profile-incidents-holder").empty();
            if (table["incidents"]) {
                $.each(table["incidents"], function(index, value) {
                    $('.profile-incidents-holder').prepend(`<div class="white-tag profile-incidents-incident" data-id="${index}">#${index} - ${value} </div>`);
                })
            }
            ...
            const tableProfiles = e.data
            $(".warrants-items").empty()
            $.each(tableProfiles, function(index, value) {
            ...
            const tableVehicles = e.data2
            var reportName = "Издирвано МПС"
            $.each(tableVehicles, function(index, value) {
                $('.warrants-items').prepend(
                    ...
                    <div class="bolo-bottom-holder">
                        ...
                        <div class="bolo-report-vehicleplate">Рег. Номер: ${value.license_plate}</div>
            ...
            $.each(table.reverse(), function(index, value) {
            ...
            $.each(table, function(index, value) {
                $('.incidents-items').prepend(
                    ...
                    <div class="incidents-top-holder">
                        ...
                        <div class="incedent-report-name">Доклад</div>
                    </div>
        ...
        } else if (e.type == "weaponsearch") {
            let CitizenTable = e.CitizenData
            let IncidentTable = e.IncidentData
            canSearchForProfiles = true
            $(".weaponsearch-items-c").empty();
            $.each(CitizenTable, function(index, value) {
                $('.weaponsearch-items-c').prepend(`   
                    <div class="profile-item" data-id="${value.cid}">
                    <img src="${value.image}" class="profile-image">
                    <div style="display: flex; flex-direction: column; margin-top: 2.5px; margin-left: 5px; width: 100%; padding: 5px;">
                    <div style="display: flex; flex-direction: column;">
                    <div class="profile-item-title">${value.name}</div>
                    </div>
                    <div class="profile-bottom-info">
                    <div class="profile-id">ID: ${value.cid}</div>
                    </div>
                    </div>
                    </div>
                `);
            ...
            $(".weaponsearch-items-i").empty();
            $.each(IncidentTable, function(index, value) {
                $('.weaponsearch-items-i').prepend(
                    `<div class="incidents-item" data-id="${value.id}">
                    <div class="incidents-top-holder">
                        <div class="incidents-item-title">${value.title}</div>
                        <div class="incedent-report-name">Доклад</div>
            ...
            $(".incidents-ghost-holder").html("")
            $(".associated-incidents-tags-holder").html("")
            $(".manage-incidents-editing-title").html("В момента редактирате доклад " + table["id"]);
            $(".manage-incidents-editing-title").data("id", table['id']);
            ...
            $(".associated-vehicles-incidents-tags-holder").empty();
            $.each(table["vehicles"], function(index, value) {
                $(".associated-vehicles-incidents-tags-holder").append(
                    `<div class="associated-vehicles-incidents-tag" data-id="${value.plate}">${value.plate}</div>`
                );
            })
            ...
            $(".manage-incidents-weapons-holder").empty();
            $.each(table["weapons"], function(index, value) {
                $('.manage-incidents-weapons-holder').append(`<div class="manage-incidents-weapon tag">${value}</div>`);
            })
            ...
            $.each(associateddata, function(index, value) {
                ...
                if (value.sentenced == 1) { sentencedTag = "green-tag" }

                const cid = value.cid
                if (value.associated == true) {
                    $(".incidents-ghost-holder").prepend(
                        `<div class="associated-incidents-user-container" data-id="${value.cid}"  data-name="${value.name}">
                            <div class="associated-incidents-user-title">${value.name} (#${value.cid})</div>
                            <div class="associated-incidents-user-tags-holder">
                                <div class="associated-incidents-user-tag ${warrantTag}" data-id="${value.cid}">Издирван</div>
                                <div class="associated-incidents-user-tag ${guiltyTag}" data-id="${value.cid}">Виновен</div>
                                <div class="associated-incidents-user-tag ${processedTag}" data-id="${value.cid}">Разследван</div>
                                <div class="associated-incidents-user-tag ${associatedTag}" data-id="${value.cid}">Замесен</div>
                            </div>
                            ...
                            <div class="associated-incidents-user-sentence-btn ${sentencedTag}" style="display:none;" data-id="${value.cid}">Наложи Глоба</div>
                        </div>`
                    );
                } else {
                    $(".incidents-ghost-holder").prepend(
                        `<div class="associated-incidents-user-container" data-id="${value.cid}"  data-name="${value.name}">
                            ...
                            <div class="associated-incidents-user-sentence-btn ${sentencedTag}" data-id="${value.cid}">Наложи Глоба</div>
                        </div>`
                    );
                }
        ...
        } else if (e.type == "incidentSearchVehicle") {
            let table = e.data
            $(".icidents-vehicle-search-holder").empty();
            $.each(table, function(index, value) {
                let plate = value.plate
                $(".icidents-vehicle-search-holder").prepend(
                    `
                    <div class="icidents-vehicle-search-item" data-id="${plate}" data-cid="${value.vehicle}" data-name="${plate}">
                        <img src="${value.pic}" class="icidents-vehicle-search-item-pfp">
                        <div class="icidents-vehicle-search-item-right">
                            <div class="icidents-vehicle-search-item-right-cid-title">Plate</div>
                            <div class="icidents-vehicle-search-item-right-cid-input"><span class="fas fa-id-card"></span> ${value.plate}</div>
                            <div class="icidents-vehicle-search-item-right-name-title">Name</div>
                            <div class="icidents-vehicle-search-item-right-name-input"><span class="fas fa-user"></span> ${value.owner}</div>
                        </div>
                    </div>
                    `
                )
            ...
            let table = e.data
            $(".manage-bolos-editing-title").html("Редактиране издирване на МПС " + table["dbid"]);
            ...
            $.each(table, function(index, value) {
                ...
                if (value.vehicle_state == "Normal Impound") {
                    impound = "orange-tag"
                }
                else if  (value.vehicle_state == "Police Impound") {
                    impound = "green-tag"
                }
                    ...
                    <div style="display: flex; flex-direction: column;">
                        <div class="dmv-item-title">${value.model} | ${value.plate}</div>
                            <div class="dmv-tags">
                                ...
                                <div class="dmv-tag ${impound}">Конфискуван</div>
                                <div class="dmv-tag ${bolo}">Издирван</div>
                                <div class="dmv-tag ${stolen}">Откраднат</div>
                                <div class="dmv-tag ${codefive}">Код 5</div>
                            </div>
                        </div>
                        <div class="dmv-bottom-info">
                            <div class="dmv-id">Собственик: ${value.owner} · ЕГН: ${value.cid}</div>
                        </div>
                    </div>
                </div>
                `)
            ...
            if (table.vehicle_state == "Normal Impound") {
                impound = "orange-tag"
            } else if  (table.vehicle_state == "Police Impound") {
                impound = "green-tag"
            }
            ...
            $('.vehicle-tags').append(`<div class="vehicle-tag ${impound}">Конфискуван</div>`);
            $('.vehicle-tags').append(`<div class="vehicle-tag ${bolo}">Издирван</div>`);
            $('.vehicle-tags').append(`<div class="vehicle-tag ${stolen} stolen-tag">Откраднат</div>`);
            $('.vehicle-tags').append(`<div class="vehicle-tag ${codefive} code5-tag">Код 5</div>`);
            $(".vehicle-info-imageurl-input").val(table["image"]);
        ...
        } else if (e.type == "getAllCams") {
            let table = e.data
            $(".cam-items").empty();
            $.each(table, function(index, value) {
                if (value.isOnline) {
                    $('.cam-items').append(`<div class="cam-item">${index+1} ${value.label} <i class="fas fa-video cam-camera-icon" onclick="OpenCamera(${index+1})"></i><i class="fas fa-location-arrow cam-arrow" onclick="Setlocation(${value.x}, ${value.y})"></i></div>`)
                } else {
                    $('.cam-items').append(`<div class="cam-item">${index+1} ${value.label} <i class="fas fa-video cam-camera-icon-offline" onclick="OpenCamera(${index+1})"></i><i class="fas fa-location-arrow cam-arrow" onclick="Setlocation(${value.x}, ${value.y})"></i></div>`)
                }
            })
...
function fidgetSpinner(page) {
    ...
    if (page == '.cam-page-container') {
        $.post(`https://${GetParentResourceName()}/getAllCams`, JSON.stringify({}));
    }
...
function addWeapon(WeaponID) {
    $('.weapons-holder').prepend(`<div class="weapon">${WeaponID}</div>`);
...
function addTag(Tag) {
    $('.tags-holder').prepend(`<div class="tag">${Tag}</div>`);
...
function expandImage(url) {
    ...
    $(".gallery-image-div").fadeIn(150);
    $(".gallery-image-div").css("display", "block");
