...
AddEventHandler('onResourceStart', function(resourceName)
    if (GetCurrentResourceName() == resourceName) then
        Citizen.SetTimeout(1000, function()
            ...
            callSign = tostring(PlayerData.metadata["callsign"])
            TriggerServerEvent('erp-dispatch:getCalls')
        end)
    end
end)
...
local tabletRot = vector3(10.0, 160.0, 360.0)
...
AddEventHandler('np-mdt:dashboardWarrants', function(sentData, sentData2)
    SendNUIMessage({ type = "warrants", data = sentData, data2 = sentData2 })
end)
...
RegisterNetEvent('np-mdt:dashboardQuoteOfTheDay')
AddEventHandler('np-mdt:dashboardQuoteOfTheDay', function(sentData)
    SendNUIMessage({ type = "quote", data = sentData })
end)
...
AddEventHandler('np-mdt:newBulletin', function(ignoreId, sentData, job)
    ...
    if job == 'police' and PlayerData.job.name == "police" then
...
AddEventHandler('np-mdt:open', function(jobLabel, lastname, firstname)
    ...
    SendNUIMessage({ type = "data", name = "Здравейте, " ..jobLabel..' '..lastname, location = playerStreetsLocation, fullname = firstname..' '..lastname })
end)
...
RegisterNUICallback("newWeapon", function(data, cb)
    if data.id ~= "" and data.weapon ~= "" then
        TriggerServerEvent('np-mdt:newWeapon', data.id, data.weapon)
    end
    cb(true)
end)

RegisterNUICallback("removeProfileWeapon", function(data, cb)
    local cid = data.cid
    local Weapon = data.text
    TriggerServerEvent('np-mdt:removeProfileWeapon', cid, Weapon)
    cb(true)
end)
...
AddEventHandler('np-mdt:getProfileData', function(sentData)
    ...
    for i = 1, #vehicles do
        sentData['vehicles'][i]['plate'] = string.upper(sentData['vehicles'][i]['plate'])
        local tempModel = vehicles[i]['vehicle']
...
RegisterNUICallback("searchWeapons", function(data, cb)
    ...
    TriggerServerEvent('np-mdt:searchWeapons', WeaponID)
    cb(true)
end)
...
RegisterNetEvent('np-mdt:getWeapons')
AddEventHandler('np-mdt:getWeapons', function(IncidentData, CitizenData)
    SendNUIMessage({ type = "weaponsearch", CitizenData = CitizenData, IncidentData = IncidentData})
end)
...
RegisterNUICallback("incidentSearchVehicle", function(data, cb)
    local plate = data.plate
    TriggerServerEvent('np-mdt:incidentSearchVehicle', plate)
    cb(true)
end)

RegisterNetEvent('np-mdt:incidentSearchVehicle')
AddEventHandler('np-mdt:incidentSearchVehicle', function(sentData)
    SendNUIMessage({ type = "incidentSearchVehicle", data = sentData })
end)
...
RegisterNUICallback("sentencePlayer", function(data, cb)
    TriggerServerEvent("np-mdt:sentencePlayer", data)
    cb('ok')
end)
...
AddEventHandler('np-mdt:searchVehicles', function(sentData)
    for i=1, #sentData do
        local vehicle = json.decode(sentData[i]['mods'])
        sentData[i]["cid"] = sentData[i].citizenid
        ...
        sentData[i]['model'] = GetLabelText(GetDisplayNameFromVehicleModel(sentData[i]['vehicle']))
    end
...
RegisterNUICallback("getAllCams", function(data, cb)
    local sentData = exports['surveillanceSystem']:SurveillanceSystemGetCams()
    SendNUIMessage({ type = "getAllCams", data = sentData })
    cb(true)
end)

RegisterNUICallback("OpenCam", function(data, cb)
    ExecuteCommand('cam '..data.camid..'')
    cb('ok')
end)

RegisterNUICallback("SetLocation", function(data, cb)
    SetNewWaypoint(tonumber(data.x), tonumber(data.y))
    Framework.Functions.Notify('Локацията бе зададена на вашия GPS')
    cb('ok')
end)
...
RegisterNUICallback("deleteIncident", function(data, cb)
    TriggerServerEvent('np-mdt:deleteIncident', data)
    cb(true)
end)
...
RegisterNUICallback("refreshWarrants", function(data, cb)
    TriggerServerEvent('np-mdt:refreshWarrants')
    cb(true)
end)
...
RegisterNUICallback("dispatchNotif", function(data, cb)
    ...
    if mentioned then
        TriggerEvent('nocore-phone:client:Notify', {
...
RegisterKeyMapping('nomdt', 'Open Police Terminal', 'keyboard', 'k')

RegisterCommand("nomdt", function()
    TriggerServerEvent("np-mdt:show")
end)

RegisterCommand("getCalls", function()
    TriggerServerEvent('erp-dispatch:getCalls')
end)