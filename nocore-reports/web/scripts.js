...
function checkIfEmpty() {
	const subject = document.getElementById("feedback_subject-input").value;
	const info = document.getElementById("feedback_information-input").value;
	const words = info.trim().split(" ");
	const subWords = subject.trim().split(" ");
  
	if (subject.trim() === "" || subWords.length < 4 || info.trim() === "" || words.length < 25) {
...
$(document).on('click', "#submitFeedback", function() {
	...
	$.post('https://nocore-reports/action', JSON.stringify({
		action: "newFeedback",
		subject: subject,
...
function viewFeedbackWindow(feedbackid) {
	...
	if (data.category === 'player_report') {
		...
		feedbackWcard.find(".feedback_category").html(`<span><i class="fas fa-question"></i> ВЪПРОС</span>`)
	} else {
		feedbackWcard.find(".feedback_category").html(`<span><i class="fas fa-bug"></i> БЪГ</span>`)
	}
}
...
function reloadFeedbackTable() {
	...
	$.each(FeedbackTable, function(id, feedback) {
		...
		if (feedback.category === 'player_report') {
			...
			category = "<span><i class='fas fa-question'></i> ВЪПРОС</span>"
		} else {
			category = "<span><i class='fas fa-bug'></i> БЪГ</span>"
		}
		...
		if (feedback.concluded != true) {
			feedbackTable.append(`
				<tr class="table-row" feedbackID="${id}">
					<td>РЕПОРТ #${feedback.feedbackid}</th>
					<td>${category}</td>
	...
	if ($(".feedbackTable .table").children().length <= 0) {
		$(".feedbackTable .feedback_subtitle").html("Брао уе, не вярвах, че ще свършите нещо.");
		...
		$(".feedback_subtitle").html("Аре работи уе, цървул...");
