...
RegisterNetEvent('nocore-carwash:client:washCar', function()
    local PlayerPed = PlayerPedId()
    local PedVehicle = GetVehiclePedIsIn(PlayerPed)
    washingVehicle = true
    Framework.Functions.Progressbar("search_cabin", "Vehicle is being washed ..", math.random(4000, 8000), false, true, {
        ...
        Framework.Functions.Notify("Washing canceled ..", "error")
        washingVehicle = false
    end)
end)
                            ...
                            if not washingVehicle then
                                ...
                                if IsControlJustPressed(0, 38) then
                                    if dirtLevel > Config.DirtLevel then
                                        TriggerServerEvent('nocore-carwash:server:washCar')
                                    else
                                        Framework.Functions.Notify("The vehicle isn't dirty", 'error')
                                    end
                                end
                            else
