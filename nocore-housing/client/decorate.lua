	...
	while true do
		...
		if DecoMode then
			...
			if GetDistanceBetweenCoords(camPos.x, camPos.y, camPos.z, Config.Houses[Currenthouse]['Coords']['Enter']['X'], Config.Houses[Currenthouse]['Coords']['Enter']['Y'], Config.Houses[Currenthouse]['Coords']['Enter']['Z'], false) > 50.0 then
				DisableEditMode()
				closeDecorateUI()
				Framework.Functions.Notify("Излезли сте извън района.")
			end
		end
	end
end)
...
RegisterNetEvent("nocore-housing:client:decorate")
AddEventHandler("nocore-housing:client:decorate", function()
	Citizen.Wait(500)
	if IsInHouse then 
		Framework.Functions.TriggerCallback('nocore-housing:server:has:house:key', function(HasHouseKey)
			if HasHouseKey then
			   ...
			   Framework.Functions.Notify("Трябва да имате ключовете от къщата.", "error")
			end
   		end, Currenthouse)
	else
		Framework.Functions.Notify("Не сте в къща.", "error")
	end
end)
...
RegisterCommand('closedecoration', function()
	DisableEditMode()
	closeDecorateUI()
	Framework.Functions.Notify("Излезе от декориране.")
end)

RegisterNetEvent("nocore-housing:server:sethousedecorations")
AddEventHandler("nocore-housing:server:sethousedecorations", function(house, decorations)
...
function LoadDecorations(house)
	if Config.Houses[house]['Decorations'] == nil or next(Config.Houses[house]['Decorations']) == nil then
		Framework.Functions.TriggerCallback('nocore-housing:server:get:decorations', function(result)
...
function SaveDecorations()
	if Currenthouse ~= nil then
		...
		TriggerServerEvent("nocore-housing:server:save:decorations", Currenthouse, ObjectList)
	end
end
...
function CheckObjMovementInput()
    ...
    if IsControlPressed( 1, Keys["Q"]) or IsDisabledControlPressed(1, Keys["Q"]) then
    	SelObjPos.z = SelObjPos.z + zVect
    end

    if IsControlPressed( 1, Keys["E"]) or IsDisabledControlPressed(1, Keys["E"]) then
    	SelObjPos.z = SelObjPos.z - zVect
    end
...
function CheckMovementInput()
	...
	if IsControlJustReleased(0, Keys["LEFTSHIFT"]) then
		...
		Framework.Functions.Notify("скоростта е ".. tostring(speeds[curSpeed]))
	end
